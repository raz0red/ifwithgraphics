...
static int  ifwg_desc_len = 0;

const char *ifwg_dumb_get_description (void) { return ifwg_desc_buf; }

void ifwg_dumb_reset_description (void)
{
    ifwg_desc_len = 0;
    ifwg_desc_buf[0] = '\0';
}

static void ifwg_desc_append (char c)
{
    if (ifwg_desc_len < IFWG_DESC_CAP - 1) {
        ifwg_desc_buf[ifwg_desc_len++] = c;
        ifwg_desc_buf[ifwg_desc_len] = '\0';
    }
}
